...
if (!isset($body->passportNo) && !isset($body->phone)) {
    echo json_encode(["status" => "fail", "msg" => "International Passport Number is required"]);
    exit();
}

$body->ref = $body->ref ?? 'PASSPORT_' . time();
$verificationResult = $controller->verifyMyPassport($body, []);

echo json_encode([
    "status" => $verificationResult['status'] ?? 'fail',
    "msg" => $verificationResult['msg'] ?? ($verificationResult['status'] == 'success' ? 'Passport verification completed' : 'Verification failed'),
    "ref" => $body->ref,
