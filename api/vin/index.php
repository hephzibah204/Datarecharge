...
if (!isset($body->vin) && !isset($body->phone)) {
    echo json_encode(["status" => "fail", "msg" => "VIN (Voter Identification Number) is required"]);
    exit();
}

$body->ref = $body->ref ?? 'VIN_' . time();
$verificationResult = $controller->verifyMyVoterCard($body, []);

echo json_encode([
    "status" => $verificationResult['status'] ?? 'fail',
    "msg" => $verificationResult['msg'] ?? ($verificationResult['status'] == 'success' ? 'Voter Card verification completed' : 'Verification failed'),
    "ref" => $body->ref,
