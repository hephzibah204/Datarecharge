...
if (!isset($body->licenseNo) && !isset($body->phone)) {
    echo json_encode(["status" => "fail", "msg" => "Driver's License Number is required"]);
    exit();
}

$body->ref = $body->ref ?? 'DL_' . time();
$verificationResult = $controller->verifyMyLicense($body, []);

echo json_encode([
    "status" => $verificationResult['status'] ?? 'fail',
    "msg" => $verificationResult['msg'] ?? ($verificationResult['status'] == 'success' ? 'Driver License verification completed' : 'Verification failed'),
    "ref" => $body->ref,
