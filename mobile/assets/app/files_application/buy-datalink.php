...
$isTwoStep   = (
    ...
    stripos($host, 'bilal') !== false
);

/* >>> ADDED: provider flags for your 3 new APIs <<< */
$isAmzaet     = (stripos($host, 'amzaet.com') !== false);
$isAlrahuz    = (stripos($host, 'alrahuzdata.com.ng') !== false);
$isVtunaija   = (stripos($host, 'vtunaija.com.ng') !== false);

/* ================= Payloads ================= */

/* Default payload (many vendors) */
$payload_default = [
...
/* >>> ADDED: amzaet payload (uses dataplan + phone) <<< */
$payload_amzaet = [
    'network'     => (int)$network_id,
    'dataplan'    => (int)$plan_id,
    'phone'       => $mobile_number,
    'request-id'  => $request_id,
];

/* >>> ADDED: alrahuzdata payload (uses networkCode/planCode/phoneNumber) <<< */
$payload_alrahuz = [
    'networkCode' => (int)$network_id,
    'planCode'    => (int)$plan_id,
    'phoneNumber' => $mobile_number,
    'request-id'  => $request_id,
];

/* >>> ADDED: vtunaija payload (mobile_number/plan/network) <<< */
$payload_vtunaija = [
    'network'       => (int)$network_id,
    'plan'          => (int)$plan_id,
    'mobile_number' => $mobile_number,
...
try {
    ...
    /* >>> ADDED: AMZAET (Token) <<< */
        ...
        curl_setopt_array($ch, [
            ...
            CURLOPT_POST           => true,
            CURLOPT_HTTPHEADER     => $headers,
            CURLOPT_POSTFIELDS     => json_encode($payload_amzaet),
        ]);
        $raw = curl_exec($ch);
        if ($raw === false) {
            $err = curl_error($ch);
            curl_close($ch);
            logToFile("AMZAET cURL error: $err");
            throw new RuntimeException("API connection failed: $err");
        }
        curl_close($ch);
        logToFile("API Response (AMZAET): $raw");
        $resp = json_decode($raw, true) ?: ['raw' => $raw];

    /* >>> ADDED: ALRAHUZ (Bearer) <<< */
    } else if ($isAlrahuz) {
        $headers = [
            "Authorization: Bearer {$apikey}",
            "Content-Type: application/json",
        ...
        curl_setopt_array($ch, [
            ...
            CURLOPT_POST           => true,
            CURLOPT_HTTPHEADER     => $headers,
            CURLOPT_POSTFIELDS     => json_encode($payload_alrahuz),
        ]);
        $raw = curl_exec($ch);
        if ($raw === false) {
            $err = curl_error($ch);
            curl_close($ch);
            logToFile("ALRAHUZ cURL error: $err");
            throw new RuntimeException("API connection failed: $err");
        }
        curl_close($ch);
        logToFile("API Response (ALRAHUZ): $raw");
        $resp = json_decode($raw, true) ?: ['raw' => $raw];

    /* >>> ADDED: VTUNAIJA (Token) <<< */
    } else if ($isVtunaija) {
        $headers = [
            "Authorization: Token {$apikey}",
            "Content-Type: application/json",
        ...
        curl_setopt_array($ch, [
            ...
            CURLOPT_POST           => true,
            CURLOPT_HTTPHEADER     => $headers,
            CURLOPT_POSTFIELDS     => json_encode($payload_vtunaija),
        ]);
        $raw = curl_exec($ch);
        if ($raw === false) {
            $err = curl_error($ch);
            curl_close($ch);
            logToFile("VTUNAIJA cURL error: $err");
            throw new RuntimeException("API connection failed: $err");
        }
        curl_close($ch);
        logToFile("API Response (VTUNAIJA): $raw");
        $resp = json_decode($raw, true) ?: ['raw' => $raw];
    ...
    jsonOut(["success" => false, "message" => $e->getMessage()], 200);
}
...
/* >>> ADDED: AMZAET normalize <<< */
} elseif ($isAmzaet) {
    $succ = strtolower((string)($resp['success'] ?? ''));
    $st   = strtolower((string)($resp['status'] ?? ''));
    if ($succ === 'true' || in_array($st, ['success','successful','ok'])) $isSuccess = true;
    if (in_array($st, ['processing','pending','process'])) $isPending = true;
    $reasonMsg = $resp['message'] ?? ($resp['msg'] ?? null);

/* >>> ADDED: ALRAHUZ normalize <<< */
} elseif ($isAlrahuz) {
    $respKey = strtolower((string)($resp['response'] ?? ''));
    $st      = strtolower((string)($resp['status'] ?? ''));
    if ($respKey === 'ok' || in_array($st, ['success','successful'])) $isSuccess = true;
    if (in_array($st, ['processing','queued','pending'])) $isPending = true;
    $reasonMsg = $resp['message'] ?? ($resp['msg'] ?? null);

/* >>> ADDED: VTUNAIJA normalize <<< */
} elseif ($isVtunaija) {
    $st = strtolower((string)($resp['status'] ?? ''));
    if (in_array($st, ['successful','success','true'])) $isSuccess = true;
    if (in_array($st, ['pending','processing','process'])) $isPending = true;
    $reasonMsg = $resp['message'] ?? ($resp['msg'] ?? null);
...
/* ================= Record + debit =================
   Policy:
   - Success  => debit immediately, status=0.
...
jsonOut(["success" => false, "message" => $msg, "provider_response" => $resp], 300)
?>