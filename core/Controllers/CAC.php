...
class CAC extends ApiAccess{
    ...
    public function verifyMyCAC($body, $networkDetails){
        ...
        if ($status === 'Off') {
            return ["status" => "fail", "msg" => "CAC service is currently disabled"];
        }

        $load = json_encode([
            "rcNumber" => $body->rcNumber ?? $body->phone,
            "consent" => true
        ]);
        ...
        if ($result->status == true || $result->status == 'success') {
            ...
            return ["status" => "fail", "msg" => $result->message ?? "Unable to verify RC number"];
        } else {
